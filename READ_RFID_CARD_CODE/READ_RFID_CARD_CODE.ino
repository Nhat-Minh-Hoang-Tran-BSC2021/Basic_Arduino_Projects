...
unsigned char master[1] = {896607449}; // Mã Card phù hợp để mở cửa
unsigned char slave[1] = {3731807019}; // Mã Card phù hợp để đóng cửa
...
void loop() {
  ...
  Serial.print(uidDec);
  if(uidDec == 896607449){
    myservo.write(random(0,45));
    delay(200);
  }else{
  }
  //--------------------------------
  if(uidDec == 3731807019){
    myservo.write(random(0,120));
    delay(2000);
  }else{
  }
  Serial.println("]");
